...
#include "Script.h"
//TODO - #include "DisplayScriptState.h"
...
DisplayScriptState *displayScriptState = new DisplayScriptState();

ScriptAction *scrollAction;
Script *script;
...
bool SagarDisplay::setup(){
	...
	nextTransitionTime = lastTransitionTime + SagarDisplay::clockDurationHighMicros;
        ...
        script = new Script(Script::repeat);
        scrollAction = new ScrollScriptAction(script, 1, 100, 1);
        script->addScriptAction(scrollAction);
        script->resetScript();
        script->setScriptState(displayScriptState);
...
void SagarDisplay::loop(){
	...
	unsigned long now = micros();
        ...
        if (displayScriptState->isUpdated) {
            displayScriptState->isUpdated = false;
          
            int idx = displayScriptState->scrollOffset;
            
            for (int col = 0; WIDTH != col; ++col){
              if (idx == data->getWidth()){
                idx = 0;
              }
//printf("Need to set col:%i to data: %i\n", col, idx);
              pixelPins[2*col]->setDataWord(data->get(idx));
              pixelPins[2*col+1]->setDataWord(data->get(idx));
      
              ++idx;
            }
            
            if (idx >= data->getWidth()){
              displayScriptState->scrollOffset = 0;
            }
        }
